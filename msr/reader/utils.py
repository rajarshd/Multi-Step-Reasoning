...
def load_words(args, examples):
    ...
    words = set()
    qids = list(examples.questions.keys())
    for qid in tqdm(qids):
        question = examples.questions[qid]
        question_text = question.text
        context_text = []
        pids_for_question = question.pids
        paras = [examples.paragraphs[pid] for pid in pids_for_question]
        for p in paras:
            context_text += p.text
        _insert(question_text)
        _insert(context_text)
    return words


def build_word_dict(args, train_exs, dev_exs):
    ...
    if not args.create_vocab:
        ...
        word_dict.tok2ind = json.load(open(os.path.join(args.vocab_dir, 'tok2ind.json')))
        word_dict.ind2tok = json.load(open(os.path.join(args.vocab_dir, 'ind2tok.json')))
        return word_dict  #
        # return the cached one
    for w in load_words(args, train_exs):
        word_dict.add(w)
    for w in load_words(args, dev_exs):
        word_dict.add(w)
